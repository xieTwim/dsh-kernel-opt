	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			"pop.supervise": "外部监督",
			"pop.model": "监督模型",
			"pop.supNote": "已开启：每轮开始前由监督模型复审运行状态，建议自动转交 Agent。",
			"pop.footer": "启动后将立即驱动 Agent 开始工作，每轮结束后自动续跑；输入框中的草稿不会被自动发送。优化曲线与完整记录见「评测」页。",
			"sup.on": "on",
			"sup.off": "off",
			...
			"ctl.budget": "循环次数",
			"advice.title": "监督记录",
			"advice.waiting": "监督已开启：每当一轮结束、循环驱动下一轮之前，监督模型会复审整个运行过程，建议自动转交 Agent 并记录在此。",
			"advice.round": "第 {n} 轮",
			"advice.earlier": "（更早循环的 {n} 条监督记录未显示，完整历史保留在会话日志中）",
