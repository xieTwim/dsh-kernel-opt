...
function project(sessionId, events, config = DEFAULT_PROJECTION) {
	...
	const finalizedIds = /* @__PURE__ */ new Map();
	...
	*/
	...
	for (const event of events) {
		...
		if (call !== null) {
			...
			if (matchesTool(call.name, config.finalizeTools)) {
				...
				if (typeof artifactRaw === "string" && artifactRaw.length > 0) finalizedArtifacts.push({
	...
	const mark = (point, seq) => {
		point.finalized = true;
		if (point.finalizeSeq === void 0 || seq > point.finalizeSeq) point.finalizeSeq = seq;
	};
	for (const point of iterations) {
		if (point.evaluationId === void 0) continue;
		const seq = finalizedIds.get(point.evaluationId);
		if (seq !== void 0) mark(point, seq);
	}
	for (const { artifact, seq } of finalizedArtifacts) {
		...
		if (best !== void 0) mark(best, seq);
	}
	let bestIndex = null;
	for (let i = 0; i < iterations.length; i += 1) {
		const point = iterations[i];
		if (point === void 0 || !eligibleBest(point)) continue;
		const best = bestIndex === null ? void 0 : iterations[bestIndex];
...
//#endregion
export { samePath as C, runHeadline as S, WRAPUP_CLOSE_LINE as _, AUDIT_CLOSE_LINE as a, inWrapUpPhase as b, CONTINUE_TRAILER as c, MODELS_PATH as d, PRESET_ID as f, SERIES_PATH as g, REVIEW_OK_LINE as h, sessionLog as i, CONTROL_PATH as l, REVIEW_HEADER as m, hasUserTask as n, AUDIT_LINE_PREFIX as o, REPLAY_LINE_PREFIX as p, project as r, CHALLENGE_LINE as s, DEFAULT_PROJECTION as t, LOOP_LINE_PREFIX as u, WRAPUP_LINE_PREFIX as v, referenceDrift as x, impliedReferences as y };
