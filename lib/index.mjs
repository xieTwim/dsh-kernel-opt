import { S as runHeadline, _ as WRAPUP_CLOSE_LINE, a as AUDIT_CLOSE_LINE, b as inWrapUpPhase, c as CONTINUE_TRAILER, d as MODELS_PATH, g as SERIES_PATH, h as REVIEW_OK_LINE, i as sessionLog, l as CONTROL_PATH, m as REVIEW_HEADER, n as hasUserTask, o as AUDIT_LINE_PREFIX, r as project, s as CHALLENGE_LINE, t as DEFAULT_PROJECTION, u as LOOP_LINE_PREFIX, v as WRAPUP_LINE_PREFIX, x as referenceDrift, y as impliedReferences } from "./chunk-projection-CHRX3BJK.mjs";
...
function completedEvals(series) {
	return series.iterations.filter((p) => p.pending !== true && p.channel !== "replay" && !inWrapUpPhase(series.rounds, p.seq)).length;
}
/** Human-readable language name used in model instructions. */
function runLanguageName(language) {
...
*/
function withRunLanguage(text, language) {
	if (language === void 0) return text;
	return `${text}\n\nRUN LANGUAGE: Write every user-facing reply and every kernel_plan/kernel_env text field in ${runLanguageName(language)}. Keep code, commands, identifiers, and protocol markers unchanged.`;
}
/**
	...
	*/
	const effectiveSupervisor = (state) => {
		...
		if (config.supervisor !== void 0) return {
			...
			...config.supervisor.reasoningEffort === void 0 ? {} : { reasoningEffort: config.supervisor.reasoningEffort },
			source: "config"
		};
	};
	...
	*/
	const loopOps = {
		...
		setSupervisorRoute: (sessionId, route) => {
			const state = stateFor(sessionId);
			if (route === void 0) delete state.supervisorOverride;
		...
		status: (sessionId) => {
			...
			return {
				...
				...state?.outputLanguage !== void 0 ? { outputLanguage: state.outputLanguage } : {},
				...state?.stopReason !== void 0 ? { stopReason: state.stopReason } : {},
				...supervisor !== void 0 ? { supervisor } : {}
			};
		}
	};
	...
	ctx.inject(["llm"], (lctx) => {
		...
		*/
				...
				const ask = async (thinking) => {
					...
					const stream = lctx.llm.stream({
						...
						system: supervisorSystem(mode === "headroom" ? HEADROOM_SYSTEM : SUPERVISOR_SYSTEM, {
							...state.outputLanguage !== void 0 ? { language: runLanguageName(state.outputLanguage) } : config.supervisor?.language !== void 0 ? { language: config.supervisor.language } : {},
						...
						...thinking ? supervisor.reasoningEffort === void 0 ? {} : { reasoningEffort: ReasoningEffortId(supervisor.reasoningEffort) } : { reasoningEffort: ReasoningEffortId("off") },
		...
		const checkpoint = async (sessionId) => {
				...
				if (challengeable || state.supervise && unreviewedEvals(series)) {
					...
					if (challengeable && advice !== null) {
						...
						agent.followup(createUserMessage({
							content: [{
								type: "text",
								text: withRunLanguage(challengeText(state.round, decision.evalsDone, state.budget, advice, finalizeHint, evalsPerTurn), state.outputLanguage)
							}],
							source: {
								kind: "plugin",
								plugin: PLUGIN_ID
					...
					if (reviewed) agent.followup(createUserMessage({
						content: [{
							type: "text",
							text: withRunLanguage(finalAuditText(advice, note), state.outputLanguage)
						}],
						source: {
							kind: "plugin",
							plugin: PLUGIN_ID
			...
			if (decision.action === "wrap-up") {
				...
				agent.followup(createUserMessage({
					content: [{
						type: "text",
						text: withRunLanguage(wrapUpText(decision.evalsDone, state.budget, decision.reason, finalizeHint, advice, reviewed && advice === null, note), state.outputLanguage)
					}],
					source: {
						kind: "plugin",
						plugin: PLUGIN_ID
			...
			state.lastAdvice = advice ?? state.lastAdvice;
			if (!state.armed || lctx.agents.get(SessionId(sessionId)) !== agent || agent.status !== "idle") return;
			const taskKnown = series.iterations.length > 0 || series.plans.length > 0 || hasUserTask(sessionLog(session));
			agent.followup(createUserMessage({
				content: [{
					type: "text",
					text: withRunLanguage(continuationText(state.round, decision.evalsDone, state.budget, advice, reviewed && advice === null, stagnationCount(series), finalizeHint, taskKnown, series.plans.length > 0, evalsPerTurn, note, planStale(series, evalsPerTurn), series.envs.length > 0), state.outputLanguage)
				}],
				source: {
					kind: "plugin",
					plugin: PLUGIN_ID
		...
		const armLoop = (sessionId, budget, outputLanguage) => {
			const state = stateFor(sessionId);
			state.armed = true;
			state.budget = budget;
			state.round = 0;
			...
			if (outputLanguage === void 0) delete state.outputLanguage;
	...
	ctx.inject(["webServer"], (wctx) => {
		const buildControl = (sessionId, series) => {
			...
			const ratio = claim?.latencyMs === void 0 ? void 0 : claim.speedup ?? (reference !== void 0 ? reference / claim.latencyMs : void 0);
			return {
				...claim?.latencyMs !== void 0 ? { result: {
					latencyMs: claim.latencyMs,
					finalized: headline.finalized,
					...ratio !== void 0 ? { speedup: ratio } : {}
				} } : {},
				loop: {
					armed: state?.armed ?? false,
					budget: state?.budget ?? 0,
					round: state?.round ?? 0,
					evalsDone,
					evalsOverBudget: state !== void 0 && state.budget > 0 ? Math.max(0, evalsDone - state.budget) : 0,
					available: loopOps.arm !== void 0,
					defaultBudget,
					...state?.outputLanguage !== void 0 ? { outputLanguage: state.outputLanguage } : {},
					...state?.stopReason !== void 0 ? { stopReason: state.stopReason } : {}
				},
				...
				(async () => {
					try {
						...
						if (action === "loop-arm") {
							...
							const budget = typeof raw === "number" && Number.isInteger(raw) && raw > 0 && raw <= 9999 ? raw : defaultBudget;
							const rawLanguage = body["outputLanguage"];
							if (rawLanguage !== void 0 && rawLanguage !== "zh" && rawLanguage !== "en") {
								respond(400, { error: "outputLanguage must be zh or en" });
								return;
							}
							arm(sessionId, budget, rawLanguage);
