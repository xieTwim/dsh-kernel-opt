...
*/
function continuationText(round, evalsDone, budget, advice, reviewedOk = false, stagnant = 0, finalizeHint = "run_finalize / kernel_finalize", taskKnown = true, planKnown = true, evalsPerTurn = 0) {
	...
	if (evalsPerTurn > 0) lines.push(`Pace: complete at most ${String(evalsPerTurn)} evaluations this turn, then settle and report — the loop reviews progress and drives you onward; do not finalize early just because the turn ends.`);
	return lines.join("\n");
}
/**
...
* @returns the followup text.
*/
function finalAuditText(advice) {
	...
	else lines.push("", REVIEW_OK_LINE, "", "No action needed — this note closes the run.");
	return lines.join("\n");
}
//#endregion
//#region src/index.ts
...
function apply(ctx, config = {}) {
	...
	*/
	...
	ctx.inject(["commands", "llm"], (lctx) => {
		...
		const review = async (state, series, closing = false) => {
			...
			try {
				const digest = closing ? `${supervisorDigest(series, state)}\nThe run has finalized — this is the closing audit: judge the final table and its provenance (the finalize and its replay above all); continuation advice is moot.` : supervisorDigest(series, state);
				let reply = "";
		...
		const checkpoint = async (sessionId) => {
			...
			if (decision.action === "wrap-up") {
				const { advice, reviewed } = reviewable(series) ? await review(state, series) : {
					advice: null,
					reviewed: false
				};
				state.lastAdvice = advice ?? state.lastAdvice;
				...
				agent.followup(createUserMessage({
					content: [{
						type: "text",
						text: wrapUpText(decision.evalsDone, state.budget, decision.reason, finalizeHint, advice, reviewed && advice === null)
					}],
					source: {
						kind: "plugin",
						plugin: PLUGIN_ID
			...
			agent.followup(createUserMessage({
				content: [{
					type: "text",
					text: continuationText(state.round, decision.evalsDone, state.budget, advice, reviewed && advice === null, stagnationCount(series), finalizeHint, taskKnown, series.plans.length > 0, evalsPerTurn)
				}],
				source: {
					kind: "plugin",
					plugin: PLUGIN_ID
