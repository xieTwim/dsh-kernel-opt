...
*/
function continuationText(round, evalsDone, budget, advice, reviewedOk = false, stagnant = 0, finalizeHint = "run_finalize / kernel_finalize", taskKnown = true) {
	...
	if (taskKnown) lines.push("", `${CONTINUE_TRAILER}: analyse the latest result, state the plan with kernel_plan if it changed, improve the kernel, and evaluate again.`, `If you are done or the remaining budget cannot beat the current best, finalize the result you stand behind (${finalizeHint}), then summarize.`);
	else lines.push("", `${CONTINUE_TRAILER}: the conversation carries no task yet. Inventory the WORKING DIRECTORY for the task the user prepared (prompt/task files, kernels, bench scripts) and start from what you find. If the workspace carries no task either, ask the user what to optimize and stop — never adopt anything found outside the working directory.`);
	return lines.join("\n");
}
/**
	...
	ctx.inject(["commands", "llm"], (lctx) => {
		...
		const checkpoint = async (sessionId) => {
			...
			agent.followup(createUserMessage({
				content: [{
					type: "text",
					text: continuationText(state.round, decision.evalsDone, state.budget, advice, reviewed && advice === null, stagnationCount(series), finalizeHint, taskKnown)
				}],
				source: {
					kind: "plugin",
					plugin: PLUGIN_ID
