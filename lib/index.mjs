...
function project(sessionId, events, config = DEFAULT_PROJECTION) {
	...
	const pendingJob = /* @__PURE__ */ new Set();
	...
	for (const event of events) {
		...
		if (call !== null) {
			...
			} else if (matchesTool(call.name, config.jobTools)) pendingJob.add(call.callId);
			continue;
		}
		...
		if (result !== null) {
			...
			if (pendingJob.has(result.callId)) {
				...
				if (collectResultText(result.message).includes("KERNEL_EVAL=")) uncollectedSeqs.push(event.seq);
			}
		}
	}
...
function supervisorDigest(series, state, tail = 10, evalsPerTurn = 0) {
	...
	if (series.uncollectedSeqs.length > 0) lines.push(`Uncollected: ${String(series.uncollectedSeqs.length)} background-job read(s) carried a contract line, so those evaluations are NOT in the record above and NOT counted against the budget. The bench must run in the foreground to be recorded — judge this run on what is listed, and say that measurements are missing rather than assuming the numbers above are the whole picture.`);
