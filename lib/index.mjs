...
* provenance.
*/
const REPLAY_LINE_PREFIX = "[cockpit-replay] ";
...
function trailerPayloads(text) {
	...
	for (const line of text.split("\n")) {
		...
		if (!trimmed.startsWith("KERNEL_COCKPIT_EVAL=")) continue;
		const payload = extractJsonObject(trimmed.slice(20));
...
function replayCommand(text) {
	for (const line of text.split("\n")) {
		const trimmed = line.trimStart();
		if (trimmed.startsWith("[cockpit-replay] ")) {
			const command = trimmed.slice(17).trim();
	...
	for (const event of events) {
		...
		if (result !== null) {
			...
			if (finalize !== void 0) {
				...
				if (text.includes("KERNEL_COCKPIT_EVAL=")) {
			...
			if (shell !== void 0) {
				...
				const text = collectResultText(result.message);
				if (!text.includes("KERNEL_COCKPIT_EVAL=")) continue;
				let consumed = false;
				for (const payload of trailerPayloads(text)) {
...
*/
function continuationText(round, evalsDone, budget, advice, reviewedOk = false, stagnant = 0, finalizeHint = "run_finalize / cockpit_finalize") {
	...
	lines.push("", `${CONTINUE_TRAILER}: analyse the latest result, state the plan with cockpit_plan if it changed, improve the kernel, and evaluate again.`, `If you are done or the remaining budget cannot beat the current best, finalize the result you stand behind (${finalizeHint}), then summarize.`);
	return lines.join("\n");
}
/**
...
* @returns the followup text.
*/
function wrapUpText(evalsDone, budget, reason, finalizeHint = "run_finalize / cockpit_finalize") {
	return [
...
function apply(ctx, config = {}) {
	...
	*/
	...
	const setSupervise = (sessionId, enabled) => {
		...
		if (enabled && effectiveSupervisor(state) === void 0) return "No supervisor model configured. Pick one (/supervise use <provider>/<model>, or the panel picker), or add to the kernel-cockpit plugin config: supervisor: { provider: <route>, model: <id> } — a distinct route/model from the primary.";
	...
	ctx.tools.register(defineTool({
		...
		description: "Record your FINAL kernel choice by artifact path (for evaluation pipelines without evaluator-issued ids; with an id-issuing evaluator call its own finalize instead). Call once, at the end, with the artifact you stand behind — restore it verbatim first if a later edit regressed it. When the best measurement for that artifact is self-reported, the cockpit replays the recorded benchmark command once and appends the output as the verified final measurement.",
		parameters: {
			artifact_path: {
				type: "string",
				required: true,
				description: "Path of the final artifact, as printed in its KERNEL_COCKPIT_EVAL trailer."
			},
			note: {
				type: "string",
