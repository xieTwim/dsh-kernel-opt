...
*/
function continuationText(round, evalsDone, budget, advice, reviewedOk = false, stagnant = 0, finalizeHint = "run_finalize / kernel_finalize", taskKnown = true, planKnown = true, evalsPerTurn = 0, okNote = null, planStale = false) {
	...
	if (taskKnown) {
		...
		else if (planStale) lines.push("Your last kernel_plan predates the recent evaluations — call it again with what you are actually pursuing now. kernel_plan is the ONLY channel to the human's plan panel; a progress write-up in the reply text never reaches it.");
		...
		*/
		...
		const checkpoint = async (sessionId) => {
			...
			agent.followup(createUserMessage({
				content: [{
					type: "text",
					text: continuationText(state.round, decision.evalsDone, state.budget, advice, reviewed && advice === null, stagnationCount(series), finalizeHint, taskKnown, series.plans.length > 0, evalsPerTurn, note, planStale(series, evalsPerTurn))
				}],
				source: {
					kind: "plugin",
					plugin: PLUGIN_ID
