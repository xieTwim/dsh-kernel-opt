import { C as samePath, i as sessionLog, p as REPLAY_LINE_PREFIX, r as project } from "./chunk-projection-CJ6WUH-S.mjs";
...
function applyCommands(ctx) {
	ctx.inject(["commands"], (cctx) => {
		const loop = () => cctx.kernelOptRuntime.loop;
		cctx.commands.register({
			...
			handler: (invocation) => {
				...
				const arm = ops.arm;
				if (arm === void 0) return {
					kind: "error",
					text: "loop machinery not composed (llm absent)."
				};
			...
			handler: (invocation) => {
				...
				if (raw.startsWith("use ") || raw === "use") {
					...
					if (spec === "default" || spec === "") {
						ops.setSupervisorRoute(sessionId, void 0);
						const { supervisor } = ops.status(sessionId);
						return {
							kind: "success",
							text: supervisor !== void 0 ? `Supervisor override cleared; following config: ${supervisor.provider}/${supervisor.model}.` : "Supervisor override cleared; nothing configured — /supervise use <provider>/<model> to pick one."
						};
					}
					...
					ops.setSupervisorRoute(sessionId, {
						provider: spec.slice(0, slash),
						model: spec.slice(slash + 1)
					});
					return {
						kind: "success",
						text: `Supervisor model for this session: ${spec}.${ops.status(sessionId).supervise ? "" : " Enable with /supervise on."}`
					};
				}
				const state = ops.status(sessionId);
				return {
					kind: "success",
					text: `supervisor ${state.supervise ? "on" : "off"}; ${state.supervisor !== void 0 ? `route: ${state.supervisor.provider}/${state.supervisor.model} (${state.supervisor.source})` : "not configured"}.`
				};
			}
		});
	});
