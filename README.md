...
模型一轮轮地读代码、改代码、跑评测，同一个会话里多出一个「评测」页签。页签顶部是这一轮**当前站得住的那个结果**，下面是两条线：**本轮结果**（模型每次试出来的）与**截至本轮最佳**（阶梯状，只在有更好的合格结果时上抬）——没通过验证的候选画在坐标轴下方，从不计入最佳。再往下是模型当前的方案；评测环境、监督记录、逐次明细收在「完整审计记录」里。
