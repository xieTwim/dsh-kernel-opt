...
也可以全程不打命令：面板顶部有**启动/停止按钮 + 预算输入 + 监督开关**（control 路由），与 slash 命令驱动同一份状态；**对话页**也有双态入口——未启动时 composer 工具行里有一个小「⟳ 循环」按钮（默认预算一键启动），循环中 composer 上方出现**状态条**（轮次/预算进度 + 停止按钮），停止即消失。
